...
kiem_tra_chan_le<-function(x){
  # kiểm tra input có phải số nguyên không
...
#   - Loại bỏ giá trị NA
tinh_diem_tb <- function(diem) {
  
  # kiểm tra điểm hợp lệ
  if (any(diem < 0 | diem > 10, na.rm = TRUE)) {
    stop("Điểm phải nằm trong khoảng 0 đến 10")
  }
  
  # tính trung bình và bỏ NA
  tb <- mean(diem, na.rm = TRUE)
  
  return(tb)
}
tinh_diem_tb(c(8, 7, 9, NA, 10))
# ------------------------------------------------------------------------------
